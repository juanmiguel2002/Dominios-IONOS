                ...
                <tbody class="text-gray-900 dark:text-gray-100">
                    @forelse ($dominios as $dominio)
                        @php
                            ...
                            if ($estadoTransferencia === 'REGISTRATION_IN_PROGRESS' && $creado) {
                            ...
                            // Marcar fila en rojo si faltan 30 días o menos y no es transferencia
                            if (!$estado) {
                                if ($fechaMostrar && $estadoTransferencia !== 'REGISTRATION_IN_PROGRESS') {
                                    $diasRestantes = now()->diffInDays($fechaMostrar, false);
                                    $filaAlerta = $diasRestantes <= 30
                                        ? 'bg-red-50 dark:bg-red-900/30 hover:text-red-200!important'
                                        : '';
                                } else {
                                    $filaAlerta = '';
                                }
                        ...
                        <tr class="dark:hover:bg-gray-700 hover:bg-gray-100 {{ $filaAlerta }}">
                            ...
                            <td class="border p-2 dark:border-gray-700 flex items-center gap-2">
                                @if ($fechaMostrar)
                                    ...
                                    @if (!$estado && $estadoTransferencia !== 'REGISTRATION_IN_PROGRESS')
                                        @php
                                            $diasRestantes = now()->diffInDays($fechaMostrar, false);
                                        @endphp
                                        @if ($diasRestantes <= 30)
                                    ...
                                    @endif
                                @else
                                    <span class="text-gray-400 dark:text-gray-600">Sin fecha de renovación</span>
                                @endif
                            </td>

                            <td class="border p-2 dark:border-gray-700">
                                @if ($estadoTransferencia === 'REGISTRATION_IN_PROGRESS')
