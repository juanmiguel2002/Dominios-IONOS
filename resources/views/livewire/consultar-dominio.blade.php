        ...
        @if ($dominio)
            <div class="bg-white dark:bg-gray-900 shadow-md border border-gray-200 dark:border-gray-700 p-6 rounded-lg space-y-4">
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                   ...
                   <form action="{{ route('enviar.email', ['id' => $dominio['id']]) }}" method="POST">
                        @csrf
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded transition cursor-pointer">Enviar correo de renovación</button>
                    </form>
                </div>
            </div>
        @endif
